...
int main(int argc, char const *argv[]) {
	...
	pthread_t threads[opMatrix.M1row*opMatrix.M2col];//opMatrix.M1row*opMatrix.M2col
	pthread_mutex_init(&mutex, NULL);
	printf("create struct\n");
	struct threadData data[opMatrix.M1row*opMatrix.M2col];
	printf("create threads\n");
	for(size_t i=0; i<opMatrix.M1row*opMatrix.M2col; i++){
		data[i].pos=i;
		data[i].opMatrix=&opMatrix;
		pthread_create(&(threads[i]), NULL, executeParallel, (void *)&data[i]);
	}
...
void *executeParallel(void *arg){
	...
	dataIn->opMatrix->mulParallel(dataIn->pos);
	pthread_exit(NULL);
